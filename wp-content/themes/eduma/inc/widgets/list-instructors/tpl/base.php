...
if ( ! empty( $co_instructors ) ) {
    ...
    foreach ( $co_instructors as $key => $instructor ) {
        ...
        $html .= '<div class="description">' . get_the_author_meta( 'description', $instructor["user_id"] ) . '</div>';
        $html .= '<div class="info_ins">';
        $html .= '<div class="row">';
        $html .= '<div class="col-sm-6 reviews"><span class="lnr lnr-star"></span> (' . $text_review . ')</div>';
        $html .= '<div class="col-sm-6 students"><span class="lnr lnr-users"></span> ' . $instructor["students"] . ' ' . esc_html__( 'Students', 'eduma' ) . '</div>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '</div>';
