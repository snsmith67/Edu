	...
	function thim_remove_learnpress_hooks() {
        ...
        add_action('init', function (){
            ...
            if ( ( thim_plugin_active( 'learnpress-woo-payment/learnpress-woo-payment.php' && class_exists( 'LP_Addon_Woo_Payment' ) ) ) && thim_is_version_addons_woo(3) ) {
                ...
                remove_action( 'learn-press/before-course-buttons', array( $instance_addon, 'purchase_course_notice' ) );
            ...
            if ( ( thim_plugin_active( 'learnpress-paid-membership-pro/learnpress-paid-memberships-pro.php' && thim_plugin_active( 'paid-memberships-pro/paid-memberships-pro.php' ) && class_exists( 'LP_Addon_Paid_Memberships_Pro' ) ) ) ) {
                ...
                remove_action( 'learn-press/before-course-buttons', array( $instance_addon, 'learn_press_before_course_buttons' ), 10 );
                add_action( 'thim_single_course_payment', array( $instance_addon, 'learn_press_before_course_buttons' ), 8 );
            }
...
add_action( 'learn_press_course_item_content', 'thim_content_item_lesson_media', 5 );
...
if(!function_exists('thim_change_tabs_course_profile')) {
    function thim_change_tabs_course_profile( $defaults ) {
        ...
        $defaults['settings']['priority'] = 1000;
        return $defaults;
    }
}
...
if ( ! function_exists( 'thim_course_tabs_content' ) ) {
    function thim_course_tabs_content($defaults) {
        ...
        $theme_options_data = get_theme_mods();
        $group_tab = isset($theme_options_data['group_tabs_course']) ? $theme_options_data['group_tabs_course'] : array('description', 'curriculum', 'instructor', 'students-list', 'review');
        foreach ( $defaults as $k => $v ) {
            switch ($k) {
                case 'overview':
                    $v['icon'] = 'fa-bookmark';
