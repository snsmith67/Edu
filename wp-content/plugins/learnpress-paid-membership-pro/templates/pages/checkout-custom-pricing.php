	...
	<table class="lp-pmpro-membership-list">
		...
		if ( ! empty( $list_courses ) ) {
			foreach ( $list_courses as $key => $course_item ) {
				...
				$content_item .= apply_filters( 'learn_press_pmpro_course_is_level', '<td class="list-item item-td item-check "><i class="dashicons dashicons-yes"></i></td>', $pmpro_level, 0, $course_item, $key );
				$content_item .= '</tr>';
