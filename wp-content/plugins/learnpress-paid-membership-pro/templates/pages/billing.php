...
if ( isset( $level->id ) && ! empty( $level->id ) ) {
	?>
	<p><?php printf( __( "Logged in as <strong>%s</strong>.", 'learnpress-paid-membership-pro' ), $current_user->user_login ); ?>
		<small>
			<a href="<?php echo wp_logout_url( get_bloginfo( "url" ) . "/membership-checkout/?level=" . $level->id ); ?>"><?php _e( "logout", 'learnpress-paid-membership-pro' ); ?></a>
		</small>
	</p>
	<ul>
		<li><strong><?php _e( "Level", 'learnpress-paid-membership-pro' ); ?>:</strong> <?php echo $level->name ?></li>
		<?php if ( $level->billing_amount > 0 ) { ?>
			<li><strong><?php _e( "Membership Fee", 'learnpress-paid-membership-pro' ); ?>:</strong>
				<?php
				$level = $current_user->membership_level;
				if ( $current_user->membership_level->cycle_number > 1 ) {
					printf( __( '%s every %d %s.', 'learnpress-paid-membership-pro' ), pmpro_formatPrice( $level->billing_amount ), $level->cycle_number, pmpro_translate_billing_period( $level->cycle_period, $level->cycle_number ) );
				} elseif ( $current_user->membership_level->cycle_number == 1 ) {
					printf( __( '%s per %s.', 'learnpress-paid-membership-pro' ), pmpro_formatPrice( $level->billing_amount ), pmpro_translate_billing_period( $level->cycle_period ) );
				} else {
		...
		<?php if ( $level->billing_limit ) { ?>
			<li>
				<strong><?php _e( "Duration", 'learnpress-paid-membership-pro' ); ?>:</strong> <?php echo $level->billing_limit . ' ' . sornot( $level->cycle_period, $level->billing_limit ) ?>
			</li>
		<?php } ?>
	</ul>
	<?php
...
<?php if ( pmpro_isLevelRecurring( $level ) ) { ?>
	<?php if ( $show_paypal_link ) { ?>
		...
		<p><?php _e( 'Your payment subscription is managed by PayPal. Please <a href="http://www.paypal.com">login to PayPal here</a> to update your billing information.', 'learnpress-paid-membership-pro' ); ?></p>
		...
		<form id="pmpro_form" class="pmpro_form" action="<?php echo pmpro_url( "billing", "", "https" ) ?>" method="post">
					...
					<tr>
						<td>
							...
							<div>
								...
								<small class="lite">(<?php _e( 'optional', 'learnpress-paid-membership-pro' ); ?>)</small>
							</div>
							...
							if ( $longform_address ) {
								?>
								<div>
									<label for="bcity"><?php _e( 'City', 'learnpress-paid-membership-pro' ); ?>City</label>
									<input id="bcity" name="bcity" type="text" class="input" size="30" value="<?php echo esc_attr( $bcity ) ?>" />
								</div>
								<div>
									<label for="bstate"><?php _e( 'State', 'learnpress-paid-membership-pro' ); ?>State</label>
			...
			if ( $pmpro_include_payment_information_fields ) {
				...
				<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						<th>
							<span class="pmpro_thead-name"><?php _e( 'Credit Card Information', 'learnpress-paid-membership-pro' ); ?></span>
						...
						<td>
							...
							if ( $pmpro_show_cvv ) {
								...
								<div>
									...
									} ?>" class=" <?php echo pmpro_getClassForField( "CVV" ); ?>" <?php if ( $gateway == "braintree" ) { ?>data-encrypted-name="cvv"<?php } ?> />
									<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( $cvv_template ); ?>','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e( "what's this?", 'learnpress-paid-membership-pro' ); ?></a>)
									</small>
								</div>
								<?php
							}
			...
			<div align="center">
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit" value="<?php _e( 'Update', 'learnpress-paid-membership-pro' ); ?>" />
				<input type="button" name="cancel" class="pmpro_btn pmpro_btn-cancel" value="<?php _e( 'Cancel', 'learnpress-paid-membership-pro' ); ?>" onclick="location.href='<?php echo pmpro_url( "account" ) ?>';" />
			</div>
	...
	<?php } ?>
<?php } else { ?>
	<p><?php _e( "This subscription is not recurring. So you don't need to update your billing information.", 'learnpress-paid-membership-pro' ); ?></p>
<?php } ?>
